...
function toggleColumn() {
	...
	const rows = Array.from(table.querySelectorAll(`tr :is(th:nth-child(${columnIndex}), td:nth-child(${columnIndex}))`));
...
function createCheckboxElements(columnsToShow = [], showColumns = true) {
	return new Promise((resolve, reject) => {
		...
		for (let i = 0; i < numColumns; i++) {
			...
			} else if ((columnsToShow.includes(i) && showColumns) || (!columnsToShow.includes(i) && !showColumns)) {
				checkbox.checked = true;
				label.className = "checkbox-container";
			} else {
