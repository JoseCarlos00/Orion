...
async function main() {
	try {
		const linkSelected = document.querySelector("#RMReportes > ul > li:nth-child(2) > div > ul > li.rmItem.rmLast > a");
...
function insertElementPrintVisualizador() {
	return new Promise((resolve, reject) => {
		const buttonPrint = /*html*/ `
      <div class="d-flex bd-highlight">
        <div class="p-2 bd-highlight">
          <button id="printButtonTrabajoActivo" type="button" class="btn btn-sm text-grey btn-purple mt-3">
          ${iconPrintSend}
          Imprimir
          </button>
        </div>
      ...
      `;
		...
		const elementoForToInsert = document.querySelector("#Panel15 > main > div.d-flex.justify-content-center");
